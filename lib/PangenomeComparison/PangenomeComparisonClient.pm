package PangenomeComparison::PangenomeComparisonClient;
...
PangenomeComparison::PangenomeComparisonClient
...
sub new
{
    ...
    my $self = {
	client => PangenomeComparison::PangenomeComparisonClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
	...
	method => "PangenomeComparison.build_pangenome",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
# Authentication: optional
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
	method => "PangenomeComparison.compare_genomes",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "PangenomeComparison.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for PangenomeComparison::PangenomeComparisonClient\n";
    }
    if ($sMajor == 0) {
        warn "PangenomeComparison::PangenomeComparisonClient version is $svr_version. API subject to change.\n";
    }
}
...
package PangenomeComparison::PangenomeComparisonClient::RpcClient;
